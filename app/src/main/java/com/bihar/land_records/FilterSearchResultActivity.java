 ...
 * Created by laxmi.
 */
public class FilterSearchResultActivity extends AppCompatActivity {
    ...
    String categoryId, cityId, searchText, companyName, jobType;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (NetworkUtils.isConnected(FilterSearchResultActivity.this)) {
            getSearchItem();
        } else {
            Toast.makeText(FilterSearchResultActivity.this, getString(R.string.conne_msg1), Toast.LENGTH_SHORT).show();
        }
    ...
    private void getSearchItem() {
        ...
        jsObj.addProperty("method_name", "get_search_job");
        jsObj.addProperty("user_id", UserUtils.getUserId());
        jsObj.addProperty("cat_id", categoryId);
        jsObj.addProperty("city_id", cityId);
        jsObj.addProperty("job_company_name", companyName);
        jsObj.addProperty("search_text", searchText);
        jsObj.addProperty("job_type", jobType);
    ...
    private void displayData() {
        if (mListItem.size() == 0) {
            ...
            if (isFirst) {
                isFirst = false;
                adapter = new JobAdapter(FilterSearchResultActivity.this, mListItem);
